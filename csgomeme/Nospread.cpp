...
void CNoSpread::CalcClient(Vector vSpreadVec, QAngle ViewIn, QAngle &ViewOut)
{
	Vector vecForward, vecRight, vecUp, vecSpreadDir;

	g_Math.AngleVectors(ViewIn, &vecForward, &vecRight, &vecUp);

	vecSpreadDir = vecForward + vecRight * -vSpreadVec.x + vecUp * -vSpreadVec.y;

	g_Math.VectorAngles(vecSpreadDir, ViewOut);

}

void CNoSpread::CalcServer(Vector vSpreadVec, QAngle ViewIn, Vector &vecSpreadDir)
{
	Vector vecViewForward, vecViewRight, vecViewUp;

	g_Math.AngleVectors(ViewIn, &vecViewForward, &vecViewRight, &vecViewUp);
...
void CNoSpread::GetSpreadVec(CUserCmd*pCmd, Vector &vSpreadVec)
{
	...
	CBaseCombatWeapon* localWeapon = (CBaseCombatWeapon*)LocalPlayer->GetWeapon();
	...
	float fRandInaccuracy = fRand1 * localWeapon->GetInnacuracy();
...
void CNoSpread::AngleVectors(const QAngle angles, Vector forward, Vector right, Vector up)
{
	float angle;
...
{
	QAngle Aim, qAntiSpread;

	Vector vSpreadVec, vForward, vServerSpreadForward;

	Aim = cmd->viewangles;

	g_Math.AngleVectors(Aim, vForward);
	...
	for (int i = 0; i <= 24; i++)
	{
		...
		QAngle qModifer;
		g_Math.VectorAngles(vServerSpreadForward, qModifer);
...
void CNoSpread::VectorAngles(const Vector &forward, const Vector &pseudoup, QAngle &angles)
{
	Vector left;
...
void CNoSpread::RollSpread(CBaseCombatWeapon*localWeap, int seed, CUserCmd*cmd, QAngle& pflInAngles)
{
	...
	Random->SetSeed((cmd->random_seed & 0xFF) + 1);
	...
	float fRandInaccuracy = fRand1 * localWeap->GetInnacuracy();
	...
	QAngle qViewAngles = pflInAngles;

	vSpread[0] = -vSpread[0];
	vSpread[1] = -vSpread[1];


	g_Math.AngleVectors(qViewAngles, &vForward, &vRight, &vUp);
